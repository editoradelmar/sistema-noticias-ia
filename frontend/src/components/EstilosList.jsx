...
import { Search, Plus, RefreshCw, Power, PowerOff, Edit, Trash2 } from 'lucide-react';
...
const EstilosList = () => {
  ...
  useEffect(() => { loadEstilos(); }, []);

  const loadEstilos = async () => {
    setLoading(true);
  try { const data = await estiloService.getAll(); let lista = Array.isArray(data) ? data : (Array.isArray(data?.data) ? data.data : []); setEstilos(lista); } catch (err) { console.error(err); alert('Error cargando estilos'); } finally { setLoading(false); }
  };

  const handleOpen = (e = null) => { setEditing(e); setIsModalOpen(true); };
  const handleClose = (reload = false) => { setIsModalOpen(false); setEditing(null); if (reload) loadEstilos(); };

  const handleDelete = async (id) => { if (!isAdmin()) { alert('No autorizado'); return; } if (!confirm('Eliminar estilo?')) return; try { await estiloService.delete(id); loadEstilos(); } catch (err) { alert('Error al eliminar'); } };

  const handleToggle = async (id, activoActual) => {
    try {
      await estiloService.toggleActivo(id, activoActual);
      loadEstilos();
    } catch (err) {
  ...
  const filtered = estilos.filter(s => s.nombre.toLowerCase().includes(filtro.toLowerCase()) || (s.tipo_estilo || '').toLowerCase().includes(filtro.toLowerCase()));
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-6 border border-slate-200 dark:border-slate-700">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-3xl font-bold text-slate-900 dark:text-slate-100">Estilos</h2>
            <p className="text-slate-600 dark:text-slate-400">Define estilos y reglas de redacción</p>
          </div>
          {(canEdit() || isAdmin()) && (
            <button onClick={() => handleOpen()} className="flex items-center gap-2 px-6 py-3 bg-emerald-600 dark:bg-emerald-500 text-white font-semibold rounded-lg hover:bg-emerald-700 dark:hover:bg-emerald-600 shadow-md transition-all">
              <Plus className="w-5 h-5" /> Nuevo Estilo
            </button>
          )}
        </div>
        ...
        <div className="flex gap-4">
          <div className="flex-1 relative">
            ...
            <input value={filtro} onChange={(e) => setFiltro(e.target.value)} placeholder="Buscar en los estilos..." className="w-full pl-12 py-3 pr-4 border-2 rounded-lg bg-white dark:bg-slate-900" />
          </div>
          <button onClick={loadEstilos} className="px-4 py-3 bg-slate-100 dark:bg-slate-900 rounded-lg"><RefreshCw className="w-5 h-5" /></button>
        </div>
      </div>
      ...
      {isModalOpen && <EstiloForm estilo={editing} onSave={handleClose} onCancel={handleClose} />}
    </div>
  );
};

export default EstilosList;
